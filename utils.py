...
async def replace_words(text):
    text = re.sub(r'\/', '', text)
    text = re.sub(r'\b(' + '|'.join(map(re.escape, REPLACE_WORDS)) + r')\b', '', text)
    text = re.sub(r"[!?,{}:;\"_*+().]|\s+", ' ', text).strip()
    text = re.sub(r"(?<=\w)'(?=\w)", '', text)  # Ensure no space is added
    text = re.sub(r'(season)\s*(\d+)\s*(episode)\s*(\d+)', lambda m: f"S{m.group(2).zfill(2)}E{m.group(4).zfill(2)}", text, flags=re.IGNORECASE)
    text = re.sub(r'(season|episode)\s*(\d+)', lambda m: f"{m.group(1)[0].upper()}{m.group(2).zfill(2)}", text, flags=re.IGNORECASE)
